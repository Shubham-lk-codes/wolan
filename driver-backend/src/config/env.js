...
const port = Number.parseInt(process.env.PORT ?? process.env.DRIVER_PORT ?? '5002', 10);
if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('PORT or DRIVER_PORT must be a valid port');
...
export const env = Object.freeze({
  ...
  port, mongoUri: required('MONGODB_URI'), mongoDbName,

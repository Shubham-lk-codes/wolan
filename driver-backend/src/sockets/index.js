...
export async function configureDriverSockets(httpServer, env) {
  ...
  if (env.redisUrl) {
    ...
    const handleRedisError = (error) => {
      ...
      console.warn(`Driver realtime Redis is unavailable (${error.message}); set REDIS_URL= to run without the shared adapter.`);
    };
  ...
  io.use(async (socket, next) => {
    try {
      ...
      const user = await User.findOne({ _id: payload.sub, role: 'DRIVER', deletedAt: null, status: 'ACTIVE' }).select('+tokenVersion');
  ...
  io.on('connection', (socket) => {
    ...
    socket.join(socketRooms.role('DRIVER'));
    socket.join(socketRooms.driver(socket.data.user.driverId));
  });
  return {
    publish: async (event, payload) => {
      if (payload.hubId) io.to(socketRooms.hub(payload.hubId)).emit(event, payload);
      if (payload.driverId) io.to(socketRooms.driver(payload.driverId)).emit(event, payload);

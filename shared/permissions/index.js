import { HQ_ROLES, normalizeRole, ROLES } from '../constants/index.js';
...
export const ROLE_PERMISSIONS = Object.freeze({
  ...
  [ROLES.HUB_MANAGER]: [
    ...
    'upload:create',
    'setting:read',
    'hub-setting:read', 'hub-setting:update',
  ],
...
export function hasPermission(user, permission) {
  const grants = new Set([...(ROLE_PERMISSIONS[normalizeRole(user?.role)] ?? []), ...(user?.permissions ?? [])]);
...
export function resolveHubScope(user, requestedHubId) {
  ...
  if (HQ_ROLES.includes(role)) return requestedHubId ? Object.freeze({ hubId: requestedHubId }) : Object.freeze({});
  const allowed = role === ROLES.REGIONAL_MANAGER ? [...new Set(user.assignedHubIds ?? [])] : [user.hubId];

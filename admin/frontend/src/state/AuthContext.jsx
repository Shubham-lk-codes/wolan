...
export function AuthProvider({ children }) {
  ...
  const login = useCallback(async credentials => { try { const result = await authService.login(credentials); setUser(result.user); return { ok: true }; } catch (error) { return { ok: false, message: getApiError(error) }; } }, []);
